...
class Cube():
    ...
    def recursive_solving(self, depth, prev_move=""):
        ...
        Args:
            depth: Number of moves in a row to consider
            prev_move: Previous move family (U, D, F, B, R, L) --> no two moves on the same layer in a row
        ...
        if depth > 0:
            ...
            if prev_move != "U":
                solved, string = self.turn_U().recursive_solving(depth, "U")
                ...
                solved, string = self.turn_U_prime().recursive_solving(depth, "U")
                #print(f"{string} U' accomplished (depth {depth}), Result: {solved}")
                if solved and (found == -1 or found > len(string)):
                    found = len(string)
                ...
                solved, string = self.turn_U2().recursive_solving(depth, "U")
            ...
            if prev_move != "D":
                solved, string = self.turn_D().recursive_solving(depth, "D")
                ...
                solved, string = self.turn_D_prime().recursive_solving(depth, "D")
                #print(f"{string} D' accomplished (depth {depth}), Result: {solved}")
                if solved and (found == -1 or found > len(string)):
                    found = len(string)
                ...
                solved, string = self.turn_D2().recursive_solving(depth, "D")
            ...
            if prev_move != "F":
                solved, string = self.turn_F().recursive_solving(depth, "F")
                ...
                solved, string = self.turn_F_prime().recursive_solving(depth, "F")
                #print(f"{string} F' accomplished (depth {depth}), Result: {solved}")
                if solved and (found == -1 or found > len(string)):
                    found = len(string)
                ...
                solved, string = self.turn_F2().recursive_solving(depth, "F")
            ...
            if prev_move != "B":
                solved, string = self.turn_B().recursive_solving(depth, "B")
                if solved and (found == -1 or found > len(string)):
                    found = len(string)
                    foundstring = "B_"+string
                self.turn_B_prime()

                solved, string = self.turn_B_prime().recursive_solving(depth, "B")
                if solved and (found == -1 or found > len(string)):
                    found = len(string)
                    foundstring = "B'"+string
                self.turn_B()

                solved, string = self.turn_B2().recursive_solving(depth, "B")
                if solved and (found == -1 or found > len(string)):
                    found = len(string)
            ...
            if prev_move != "R":
                solved, string = self.turn_R().recursive_solving(depth, "R")
                if solved and (found == -1 or found > len(string)):
                    found = len(string)
                    foundstring = "R_"+string
                self.turn_R_prime()

                solved, string = self.turn_R_prime().recursive_solving(depth, "R")
                if solved and (found == -1 or found > len(string)):
                    found = len(string)
                    foundstring = "R'"+string
                self.turn_R()

                solved, string = self.turn_R2().recursive_solving(depth, "R")
                if solved and (found == -1 or found > len(string)):
                    found = len(string)
            ...
            if prev_move != "L":
                solved, string = self.turn_L().recursive_solving(depth, "L")
                if solved and (found == -1 or found > len(string)):
                    found = len(string)
                    foundstring = "L_"+string
                self.turn_L_prime()

                solved, string = self.turn_L_prime().recursive_solving(depth, "L")
                ...
                solved, string = self.turn_L2().recursive_solving(depth, "L")
                if solved and (found == -1 or found > len(string)):
                    found = len(string)
...
if __name__ == "__main__":
    ...
    starttime = time.time()
    cube = generate_cube()
    print(f"generation: {time.time()-starttime} seconds")
    ...
    print(f"total time: {time.time()-totalstarttime} seconds")
